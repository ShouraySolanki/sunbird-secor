...
//					LOG.info("ji" + message.getPayload());
				...
				// If channel identifier is different for raw and summary events. Ex: context.channel/dimensions.channel
					...
					try {
						...
						LOG.warn("Unable to get path: " + JsonPath.parse(jsonObject).read("$." + channelIdentifier[1], String.class));
						channelStr = JsonPath.parse(jsonObject).read("$." + channelIdentifier[1], String.class);
					}
				}
			} catch (PathNotFoundException e) {
				LOG.warn("Unable to get path: " + e.getMessage() + "hi" + channelIdentifier);
				channelStr = "others";
			}
		}
